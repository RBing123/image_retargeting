...
void warping(unsigned int target_width, unsigned int target_height)
{
    ...
    for (unsigned int patch_index = 0; patch_index < patch_num; patch_index++) {
        ...
        for (unsigned int i = 0; i < edge_list.size(); i++) {
            ...
            // DST
            d += DST_WEIGHT*PATCH_SIZE_WEIGHT*alpha * patch[patch_index].saliency_value *
            ...
            // DLT
            d += DLT_WEIGHT*PATCH_SIZE_WEIGHT*(1 - alpha) * (1 - patch[patch_index].saliency_value) * 
                (IloPower(vp[edge.pair_indice.first * 2] - vp[edge.pair_indice.second * 2] -
...
int main(int argc, const char * argv[]) {
    ...
    source_image = cv::imread("res/gallery.jpg");
    ...
    seg_image = segmentation(source_image);
    if (!seg_image.data) {
        std::cerr << "Failed to load input image" << std::endl;
        return -1;
    }
    ...
    sal_image = cv::imread("res/gs.jpeg");
    // cv::cvtColor(seg_image, sal_image, cv::COLOR_BGR2GRAY);
    // cv::Ptr<cv::saliency::Saliency> saliencyAlgorithm = cv::saliency::StaticSaliencySpectralResidual::create();
    // cv::Mat saliencyMap;
    // bool success = saliencyAlgorithm->computeSaliency(sal_image, saliencyMap);
    // sal_image=saliencyMap;
    // sal_image = cv::saliency::StaticSaliencyFineGrained(source_image);
    // if (!sal_image.data) {
    //     std::cerr << "Failed to load input image" << std::endl;
    //     return -1;
    // }
    ...
    cv::imwrite("result/result_gs.png", result_image);
