...
Mat createEnergyImage(Mat& image) {
    ...
    if (demo) {
        namedWindow("Energy Image", cv::WINDOW_AUTOSIZE); imshow("Energy Image", energy_image);
    }
...
Mat createCumulativeEnergyMap(Mat& energy_image, SeamDirection seam_direction) {
    ...
    if (demo) {
        ...
        namedWindow("Cumulative Energy Map", cv::WINDOW_AUTOSIZE); imshow("Cumulative Energy Map", color_cumulative_energy_map);
    }
...
void reduce(Mat& image, vector<int> path, SeamDirection seam_direction) {
    ...
    if (demo) {
        namedWindow("Reduced Image", cv::WINDOW_AUTOSIZE); imshow("Reduced Image", image);
    }
...
void showPath(Mat& energy_image, vector<int> path, SeamDirection seam_direction) {
    ...
    namedWindow("Seam on Energy Image", cv::WINDOW_AUTOSIZE); imshow("Seam on Energy Image", energy_image);
}

void driver(Mat& image, SeamDirection seam_direction, int iterations) {
    clock_t start = clock();

    namedWindow("Original Image", cv::WINDOW_AUTOSIZE); imshow("Original Image", image);
    ...
    namedWindow("Reduced Image", cv::WINDOW_AUTOSIZE); imshow("Reduced Image", image); waitKey(0);
...
int main() {
    ...
    int iterations;
    filename = {"C:/Users/User/Desktop/1.jpg"};
    cv::Mat image = cv::imread("C:/Users/User/Desktop/1.jpg");
    cv::resize(image, image, Size(image.cols / 4, image.rows / 4), 0, 0, INTER_LINEAR);
    if (image.empty()) {
    ...
    //saliency map
    int numberOfColorChannels = image.channels();
    int stepSize = 33;
    cv::Mat grayImage;
    ...
    /* easy way to resize
    cv::Mat image = A.clone();
    //cv::cvtColor(image, image, cv::COLOR_BGR2RGB);
    cv::imshow("Image", image);
    resize(image, image, cv::Size(image.cols / 10, image.rows), 0, 0, INTER_LINEAR);
    int columns = image.cols;
    int lineSpacing = 5;
    for (int row = 0; row < rows; row += stepSize) {
        cv::line(image, cv::Point(0, row), cv::Point(columns, row), cv::Scalar(0, 0, 255), 1);
    }
    for (int col = 0; col < columns; col += stepSize) {
        cv::line(image, cv::Point(col, 0), cv::Point(col, rows), cv::Scalar(0, 0, 255), 1);
    }
    */

    cv::imshow("Original Image", image);
    cv::imshow("Resized Image", magnitude); //saliency map
    cv::waitKey(0);
